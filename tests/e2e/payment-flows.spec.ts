...
import { PAYMENT_SCENARIOS } from "../fixtures/payment-scenarios";
...
function buildFlowConfig(
  ...
  scenario: (typeof PAYMENT_SCENARIOS)[number],
  href: string,
): FlowConfig {
  return {
    name: `Payment ${scenarioId} — ${scenarioLabel}`,
...
/**
 * True when the failure means the chosen condition can't be exercised — retry
 * with another. Anything else (real failure on the payment step) bubbles up.
 */
...
test.describe("Payment Flows", () => {
  for (const scenario of PAYMENT_SCENARIOS) {
    test(`${scenario.id}: ${scenario.label}`, async ({ page, baseURL }) => {
      ...
      // ─── Step 2: Try each condition until payment succeeds ───────────────
      ...
      for (let i = 0; i < hrefs.length; i++) {
        ...
        try {
          ...
          if (isRetryableError(err) && i < hrefs.length - 1) {
            ...
            console.log(
              `↻ "${flowConfig.conditionName}" can't reach payment (${msg.split("\n")[0]}) — trying next`,
            );
